...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ( last_Offset_x != currentIndex && !_loadAll) {
        
        NSString * ClassString = NSStringFromClass([self.viewsArray[currentIndex] class]);
        
        NSLog(@"滑动通知 ClassString  = %@", StringFormat(@"%@%ld",ClassString , currentIndex) ) ;
        
        [[NSNotificationCenter defaultCenter] postNotificationName:ClassString object:nil userInfo:nil];
        
    }
    
    last_Offset_x = currentIndex;
