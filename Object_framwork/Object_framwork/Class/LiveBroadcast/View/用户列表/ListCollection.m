...
-(void)listReloadNoew{
    ...
    [YBToolClass postNetworkWithUrl:@"Live.getUserLists" andParameter:userlist success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        if (code == 0) {
            ...
            [self quickSort1:self.listArray];
            NSLog(@"%ld------%@",self.listArray.count,[infos valueForKey:@"nums"]);
            if ([self.delegate respondsToSelector:@selector(upUserNumMethod:)]) {
                [self.delegate upUserNumMethod:[infos valueForKey:@"nums"]];
            }
            [self->_listCollectionview reloadData];
...
-(instancetype)initWithListArray:(NSMutableArray *)list andID:(NSString *)ID andStream:(NSString *)streams{
    ...
    if (self) {
        ...
        flowlayoutt.itemSize = CGSizeMake(30,30);
        flowlayoutt.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        _listCollectionview = [[UICollectionView alloc]initWithFrame:CGRectMake(0,0,WIDTH- 130,30) collectionViewLayout:flowlayoutt];
...
-(void)setFrame:(CGRect)frame{
    ...
    _listCollectionview.frame = CGRectMake(0,0,frame.size.width,30) ;
}
