...
-(void)setModel:(chatModel *)model{
    ...
    if ([_model.titleColor isEqual:@"userLogin"]) {
        ...
        _chatLabel.text = [NSString stringWithFormat:@"%@%@",_model.userName,_model.contentChat];
    }else if ([_model.titleColor isEqual:@"zhongjiang"]){
        _chatLabel.text = [NSString stringWithFormat:@" %@ ",_model.contentChat];
    }else if ([_model.titleColor isEqual:@"xiazhu"]){
        _chatLabel.text = [NSString stringWithFormat:@" %@ ",_model.contentChat];
    }else{
        _chatLabel.text = [NSString stringWithFormat:@"%@:%@",_model.userName,_model.contentChat];
    }
    ...
    if ([_model.titleColor isEqual:@"firstlogin"]) {
        ...
        _chatLabel.textColor = [UIColor whiteColor];
        _chatLabel.font = [UIFont boldSystemFontOfSize:15];
        self.chatView.backgroundColor = RGB_COLOR(@"#000000", 0.5);
        noteStr = [[NSMutableAttributedString alloc] initWithString:_model.contentChat attributes:nil];
        
        NSTextAttachment *attch = [[NSTextAttachment alloc] init];
        // 表情图片
        attch.image = [UIImage imageNamed:@"sureSce"];
        // 设置图片大小
        attch.bounds = CGRectMake(0,-3,35,16);
        ...
        [noteStr insertAttributedString:string atIndex:0];//插入到第几个下标
        
        
    }else if ([_model.titleColor isEqual:@"xiazhu"]){//wxm 主播端投注
        _chatLabel.textColor = [UIColor whiteColor];
        _chatLabel.font = [UIFont boldSystemFontOfSize:15];
        self.chatView.backgroundColor = RGB_COLOR(@"#000000", 0.5);
        noteStr = [[NSMutableAttributedString alloc] initWithString:_model.contentChat attributes:nil];
        NSTextAttachment *attch = [[NSTextAttachment alloc] init];
        // 表情图片
        attch.image = [UIImage imageNamed:@"xitong"];
        // 设置图片大小
        attch.bounds = CGRectMake(0,-3,35,16);
        NSAttributedString *string = [NSAttributedString attributedStringWithAttachment:attch];
        NSAttributedString *speaceString = [[NSAttributedString  alloc]initWithString:@" "];
...
//    else if ([_model.titleColor isEqual:@"SendContentInfo"]){//发送联系方式
//        _chatLabel.textColor = [UIColor whiteColor];
//        _chatLabel.font = [UIFont boldSystemFontOfSize:15];
//        self.chatView.backgroundColor = RGB_COLOR(@"#000000", 0.5);
//        noteStr = [[NSMutableAttributedString alloc] initWithString:_model.contentChat attributes:nil];
//        NSTextAttachment *attch = [[NSTextAttachment alloc] init];
//        // 表情图片
//        attch.image = [UIImage imageNamed:@"danmu_xitong"];
//        // 设置图片大小
//        attch.bounds = CGRectMake(0,-3,35,16);
//        NSAttributedString *string = [NSAttributedString attributedStringWithAttachment:attch];
//        NSAttributedString *speaceString = [[NSAttributedString  alloc]initWithString:@" "];
...
//        }];
        ...
        liangAttchment.bounds = CGRectMake(0, -2, 20, 15);//设置frame
        ...
        if ([_model.isAnchor isEqual:@"1"]) {
            ...
            [noteStr addAttribute:NSForegroundColorAttributeName value:RGB(58, 104, 121) range:redRange];
        }
//        danmu_xitong
            if ([_model.titleColor isEqual:@"light0"])//青蛙
            {
                ...
                NSTextAttachment *attch = [[NSTextAttachment alloc] init];
                // 表情图片
                attch.image = [UIImage imageNamed:@"heart_1"];
                // 设置图片大小
            ...
            else if ([_model.titleColor isEqual:@"light1"])//猴子
            {
                ...
                NSTextAttachment *attch = [[NSTextAttachment alloc] init];
                // 表情图片
                attch.image = [UIImage imageNamed:@"heart_2"];
                // 设置图片大小
                attch.bounds = CGRectMake(0,-4,17,17);
                NSAttributedString *string = [NSAttributedString attributedStringWithAttachment:attch];
                [noteStr appendAttributedString:string];
            ...
            else if ([_model.titleColor isEqual:@"light2"])//小红花
            {
                ...
                NSTextAttachment *attch = [[NSTextAttachment alloc] init];
                // 表情图片
                attch.image = [UIImage imageNamed:@"heart_3"];
                // 设置图片大小
                attch.bounds = CGRectMake(0,-4,17,17);
                NSAttributedString *string = [NSAttributedString attributedStringWithAttachment:attch];
                [noteStr appendAttributedString:string];
            ...
            else if ([_model.titleColor isEqual:@"light3"])//小黄花
            {
                ...
                NSTextAttachment *attch = [[NSTextAttachment alloc] init];
                // 表情图片
                attch.image = [UIImage imageNamed:@"heart_4"];
                // 设置图片大小
                attch.bounds = CGRectMake(0,-4, 17, 17);
                NSAttributedString *string = [NSAttributedString attributedStringWithAttachment:attch];
                [noteStr appendAttributedString:string];
            ...
            else if ([_model.titleColor isEqual:@"light4"])//心
            {
                ...
                NSTextAttachment *attch = [[NSTextAttachment alloc] init];
                // 表情图片
                attch.image = [UIImage imageNamed:@"heart_5"];
                // 设置图片大小
                attch.bounds = CGRectMake(0,-4, 17, 17);
                NSAttributedString *string = [NSAttributedString attributedStringWithAttachment:attch];
                [noteStr appendAttributedString:string];
