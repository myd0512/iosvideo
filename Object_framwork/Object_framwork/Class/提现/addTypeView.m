...
- (void)sureBtnClick{
    ...
    if (showType == 1) {
        ...
        if (text2.text == nil || text2.text == NULL || text2.text.length == 0) {
            [SVProgressHUD showErrorWithStatus:@"请输入支付宝账号姓名"];
            return;
        }
    ...
    if (showType == 2) {
        if (text1.text == nil || text1.text == NULL || text1.text.length == 0) {
            [SVProgressHUD showErrorWithStatus:@"请输入微信账号"];
            return;
        }
        subdic = @{@"type":@(showType),
                   @"account":text1.text,
    ...
    if (showType == 3) {
        ...
        if (text2.text == nil || text2.text == NULL || text2.text.length == 0) {
            [SVProgressHUD showErrorWithStatus:@"请输入银行卡账号"];
            return;
        }
        ...
        subdic = @{@"type":@(showType),
                   @"account":text2.text,
                   @"name":text3.text,
                   @"account_bank":text1.text
                   };

    }
    NSMutableDictionary * dict = @{@"uid":[UserInfoManaget sharedInstance].model.id ,
        @"token":[UserInfoManaget sharedInstance].model.token }.mutableCopy;
    [dict addEntriesFromDictionary:subdic];
    [[ ZKHttpTool shareInstance ] get:[ZKSeriverBaseURL getUrlType:SetUserAccount] params:dict withHUD:NO success:^(id json) {
