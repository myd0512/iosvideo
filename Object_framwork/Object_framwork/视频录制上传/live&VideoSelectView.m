...
- (void)creatUI{
    ...
    hidBtn.frame = CGRectMake(0, 0, _window_width, _window_height-(150+kStatusBarHeight));
    [hidBtn addTarget:self action:@selector(hidBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:hidBtn];
    whiteView = [[UIView alloc]initWithFrame:CGRectMake(0, _window_height, _window_width, 150+kStatusBarHeight)];
...
- (void)showWhiteView{
    [UIView animateWithDuration:0.3 animations:^{
        whiteView.frame = CGRectMake(0, _window_height - (150+kStatusBarHeight), _window_width, 150+kStatusBarHeight);
    }];
}
- (void)hidBtnClick{
    [UIView animateWithDuration:0.3 animations:^{
        whiteView.frame = CGRectMake(0, _window_height, _window_width, 150+kStatusBarHeight);
