...
//    NSString *requestUrl = @"/?service=" ;
    NSString *requestUrl = [Base_URL stringByAppendingFormat:@"api/public/?service=%@",url];

    requestUrl = [requestUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *pDic = [[NSMutableDictionary alloc]initWithObjectsAndKeys:minstr([UserInfoManaget sharedInstance].model.id),@"uid",minstr([UserInfoManaget sharedInstance].model.token),@"token", nil];
...
//                 [MBProgressHUD showError:minstr([responseObject valueForKey:@"msg"])];
          ...
          failure:^(NSURLSessionDataTask *task, NSError *error)
     {
         ...
         NSLog(@"-=---=-=-=-=-=-=-MMMMMMMMMMMMMMMMMMMMMMMMMMMM__________MMMMMMMMMMM______");
         failBlock();
