                        ...
                        @{
                            @"msg": @[
                                    @{
                                        ...
                                        @"timestamp": @"",
                                        @"touid": @"0",
                                        @"ugood": [UserInfoManaget sharedInstance].model.id,
                                        @"uid": [UserInfoManaget sharedInstance].model.id,
                                        @"uname": [UserInfoManaget sharedInstance].model.user_nicename,
                                        @"equipment": @"app",
                                        @"uhead":[UserInfoManaget sharedInstance].model.avatar,
                                        @"level":[UserInfoManaget sharedInstance].model.level,
                                        @"vip_type":[UserInfoManaget sharedInstance].model.level,
                                        @"liangname":[UserInfoManaget sharedInstance].model.level,
                                        @"isAnchor":@"1",
                                        }
                                    ],
                            @"retcode": @"000000",
...
// 发送名片消息
-(void)sendMingPianInfoMessage:(NSString *)toid andToname:(NSString *)name{
                        ...
                        @{
                            @"msg": @[
                                    @{
                                        @"_method_": @"SystemNot",
                                        @"action": @"666",
                                        @"ct":[UserInfoManaget sharedInstance].model.contact,
                                        @"msgtype": @"4",
                                        @"touid": toid,
                                        @"toname": name,
                                        @"uid": [UserInfoManaget sharedInstance].model.id,
                        ...
                        @{
                            @"msg": @[
                                    @{
                                        ...
                                        @"uname": [UserInfoManaget sharedInstance].model.user_nicename,
                                        @"equipment": @"app",
                                        @"roomnum": [UserInfoManaget sharedInstance].model.id,
                                        @"level":[UserInfoManaget sharedInstance].model.level,
                                        @"usign":@"",
                                        @"uhead":[UserInfoManaget sharedInstance].model.avatar,
                                        @"sex":@"",
...
-(void)sendKaiJiangInfo:(NSString *)info andDaShangID:(NSString *)dsID andDSMoney:(NSString *)dsMoney{
...
/**
 机器人信息
 */
- (void)bettingInfo:(NSString *)info gameType:(NSString *)type  gameID:(NSString *)gameID gameNumnber:(NSString *)number {
...
//    ChatSocket = [[SocketIOClient alloc] initWithSocketURL:url config:@{@"log": @NO,@"forcePolling":@YES,@"reconnectWait":@1}];
    ...
    NSArray *cur = @[@{@"username":[UserInfoManaget sharedInstance].model.user_nicename,
                       @"uid":[UserInfoManaget sharedInstance].model.id,
    ...
    [ChatSocket on:@"conn" callback:^(NSArray* data, SocketAckEmitter* ack) {
        ...
        [weakself getZombie];
        if (isbusy) {
            [self.delegate checkLiveingStatus];
        }
        isbusy = YES;
    }];
    ...
    [ChatSocket on:@"broadcastingListen" callback:^(NSArray* data, SocketAckEmitter* ack) {
            ...
            [self.delegate superStopRoom:@"1"];
            UIAlertView  *alertsLimit = [[UIAlertView alloc]initWithTitle:@"涉嫌违规，被下播" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertsLimit show];
            return ;
          }
            ...
            if ([retcode isEqual:@"409002"]) {
                [SVProgressHUD showErrorWithStatus:@"你已被禁言"];
                return;
            }
...
-(void)getmessage:(NSDictionary *)msg andMethod:(NSString *)method{
    ...
    NSLog(@"socket msg = %@ , method = %@", msg , method ) ;
...
//                           }
    ...
    else if([method isEqual:@"SendGift"])
    {
        if ([minstr([msg valueForKey:@"ifpk"]) isEqual:@"1"]) {
            if ([minstr([msg valueForKey:@"roomnum"]) isEqual:[UserInfoManaget sharedInstance].model.id]) {
...
//    }
    ...
    else if([method isEqual:@"SendBarrage"])
    {
        ...
        if ([msgtype isEqualToString:@"100"]&&[action isEqualToString:@"300"]) {
            
            if (ct) {
                       [self.delegate socketSystemWithDs:ct];
                   }
            
        }else  if([msgtype isEqualToString:@"4"]&&[action isEqual:@"666"]){ // 收到 发送名片消息
                
                NSString* ct;
                _titleColor = @"SendContentInfo";
                NSString* uname = [msg valueForKey:@"uname"];
             NSString* toname = [msg valueForKey:@"toname"];
            ct = [NSString stringWithFormat:@"%@", [NSString stringWithFormat:@"%@获得了主播名片",toname]];
                NSString *levell = [msg valueForKey:@"level"];
                NSString *ID = [msg valueForKey:@"uid"];

                NSDictionary *chat = [NSDictionary dictionaryWithObjectsAndKeys:uname,@"userName",ct,@"contentChat",levell,@"levelI",ID,@"id",_titleColor,@"titleColor",nil];
                [self.delegate sendMessage:chat];
            }else{
...
//            [MBProgressHUD showError:[NSString stringWithFormat:@"%@%@",minstr([msg valueForKey:@"uname"]),YZMsg(@"已下麦")]];
        ...
        [self.delegate socketSystem:[NSString stringWithFormat:@"%@ %@",minstr([msg valueForKey:@"uname"]),@"守护了主播"]];
    }  else if ([method isEqual:@"LiveConnect"]){
        NSLog(@"LiveConnect====%@",msg);
...
//                [MBProgressHUD showError:@"游戏状态下不能进行连麦哦～"];
    ...
    else if([method isEqual:@"SendRed"])
    {
        ...
        switch (action) {
                ...
                break;
                case 3:
//                [MBProgressHUD showError:YZMsg(@"对方拒绝了你的PK申请")];
