...
- (void)creatUIWithGiftData:(NSDictionary *)giftData andVideoitem:(SVGAVideoEntity * _Nullable)videoitem{
    if (videoitem) {
        SVGAPlayer *player = [[SVGAPlayer alloc] initWithFrame:CGRectMake(0, (HEIGHT-(WIDTH/videoitem.videoSize.width*videoitem.videoSize.height))/2, WIDTH, (WIDTH/videoitem.videoSize.width*videoitem.videoSize.height))];
    ...
    NSString *titleStr = [NSString stringWithFormat:@"%@ %@%@",minstr([giftData valueForKey:@"nicename"]),@"送了一个",minstr([giftData valueForKey:@"giftname"])];
    CGFloat titleWidth = [[YBToolClass sharedInstance] widthOfString:titleStr andFont:[UIFont systemFontOfSize:14] andHeight:30];
    UIImageView *titleBackImgView = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH, 110, 35+titleWidth+20, 30)];
