...
-(void)userPlar:(NSDictionary *)dic{
    ...
    if ([dic[@"titleColor"] isEqual:@"touzhuInfo"]) {
        _touzhu = 1;
    }
    ...
    _userMoveImageV = [[UIImageView alloc]initWithFrame:CGRectMake(90,0, _window_width*0.8,60)];
    [self addSubview:_userMoveImageV];
    
    if (levelLimit.integerValue>2 && levelLimit.integerValue<11) {
      [_userMoveImageV setImage:[UIImage imageNamed:@"vipFootImage"]];
    }else if (levelLimit.integerValue>10 && levelLimit.integerValue<16){
        [_userMoveImageV setImage:[UIImage imageNamed:@"5555"]];
    }else{
        [_userMoveImageV setImage:[UIImage imageNamed:@"3333"]];
    }
    
    
    
    
    _msgView = [[UIView alloc]initWithFrame:CGRectMake(-_window_width+90, 16, _window_width, 40)];
...
//     }
    ...
    _topView = [[UIImageView alloc]initWithFrame:CGRectMake(5, 0, 90, 70)];
    [self addSubview:_topView];
    if (levelLimit.integerValue>2 && levelLimit.integerValue<11) {
      _topView.image = [UIImage imageNamed:@"vipHeadeImage"];
    }else if (levelLimit.integerValue>10 && levelLimit.integerValue<16){
        [_topView setImage:[UIImage imageNamed:@"1111"]];
    }else{
        [_topView setImage:[UIImage imageNamed:@"2222"]];
    }
    
    
    
    UIImageView *starImgView = [[UIImageView alloc]initWithFrame:CGRectMake(36, 19, 8, 8)];
    starImgView.image = [UIImage imageNamed:@"loginStar"];
    [_topView addSubview:starImgView];
  
    
    UIImageView *iconImgView = [[UIImageView alloc]initWithFrame:CGRectMake(100, 16, 18, 18)];
    iconImgView.layer.cornerRadius = 9.0;
    ...
    [iconImgView setImage:[UIImage imageNamed:@"vipHeadeImage"]];
    ...
    UIImageView *levelImage = [[UIImageView alloc]initWithFrame:CGRectMake(iconImgView.right + 4,15,28,14)];
    ...
    UILabel *nameL = [[UILabel alloc]initWithFrame:CGRectMake(iconImgView.right + 3,10,_window_width*0.8-40,30)];
    nameL.text = [NSString stringWithFormat:@"%@ 进入了直播间",[ct valueForKey:@"user_nicename"]];
    nameL.textColor = [UIColor whiteColor];
    nameL.font = [UIFont systemFontOfSize:11];
    
    NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@%@",[ct valueForKey:@"user_nicename"],@"进入了直播间"]];
    ...
    [noteStr addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Helvetica-Bold" size:10] range:redRange];
...
//    [_msgView addSubview:levelImage];
    ...
    CGPathAddArc(path, NULL, _topView.centerX-16, _topView.centerY+5, 20, 0,M_PI * 2, 0);
...
//        _userMoveImageV.frame = CGRectMake(-15,0,_window_width*0.8,50);
        _userMoveImageV.x = 95;
        _msgView.x = 90;
        _topView.x = 80;
